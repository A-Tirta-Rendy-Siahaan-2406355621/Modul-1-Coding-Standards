...
REFLEKSI 5 :
1. Menurut saya, alur Test-Driven Development (TDD) yang digunakan pada latihan ini cukup membantu dalam proses pengembangan program. Dengan menulis test terlebih dahulu sebelum implementasi, saya dapat memahami kebutuhan sistem dengan lebih jelas dan memastikan bahwa setiap fungsi yang dibuat sesuai dengan perilaku yang diharapkan. Selain itu, proses TDD yang terdiri dari siklus RED → GREEN → REFACTOR membantu saya mendeteksi kesalahan lebih awal dan membuat kode menjadi lebih terstruktur.
   Namun, masih ada beberapa hal yang dapat ditingkatkan ketika membuat test di masa depan. Saya perlu lebih memperhatikan cakupan test dan memastikan bahwa semua kemungkinan kasus, baik happy path maupun edge case, sudah diuji. Selain itu, saya juga perlu menulis test yang lebih jelas dan terorganisasi agar lebih mudah dipahami serta mempermudah proses pemeliharaan kode di kemudian hari.

2. Secara umum, unit test yang dibuat dalam tutorial sudah cukup mengikuti prinsip F.I.R.S.T. (Fast, Independent, Repeatable, Self-Validating, dan Timely). Test yang dijalankan relatif cepat karena hanya menguji bagian kecil dari program, serta menggunakan mock pada beberapa bagian sehingga tidak bergantung pada sistem lain. Test juga bersifat self-validating karena menggunakan assertion untuk menentukan apakah hasilnya benar atau salah secara otomatis.
   Namun, masih ada beberapa aspek yang bisa diperbaiki agar lebih sesuai dengan prinsip F.I.R.S.T. Misalnya, memastikan setiap test benar-benar independen dan tidak bergantung pada hasil test lain. Selain itu, penamaan test dan struktur pengujian juga bisa dibuat lebih jelas agar tujuan setiap test lebih mudah dipahami. Ke depan, saya juga perlu lebih disiplin menulis test sebelum implementasi kode agar prinsip Timely dalam F.I.R.S.T. dapat diterapkan dengan lebih baik.


REFLEKSI UNTUK BONUS 2:
1. Menurut saya, kode yang dibuat oleh partner saya sudah cukup baik dan mengikuti struktur yang diminta pada project, seperti pemisahan antara controller, service, dan model. Fitur utama yang diminta dalam tugas juga sudah dapat berjalan dengan baik. Namun, masih terdapat beberapa hal yang dapat ditingkatkan, terutama dari sisi maintainability dan penerapan best practice. Misalnya, masih terdapat penggunaan nilai yang di-hardcode langsung di dalam controller serta penggunaan field injection dengan @Autowired, yang sebenarnya kurang direkomendasikan dalam praktik pengembangan Spring modern.
2. Untuk berkontribusi pada kode partner saya, saya melakukan proses code review terhadap implementasi yang telah dibuat dan mencari bagian-bagian yang masih dapat diperbaiki. Setelah menemukan beberapa potensi perbaikan, saya membuat branch baru bernama refactor/[NPM] berdasarkan versi terbaru dari repository partner saya. Pada branch tersebut, saya melakukan beberapa perubahan refactoring untuk meningkatkan kualitas kode. Setelah itu, saya memastikan bahwa perubahan yang saya lakukan tidak merusak fitur yang sudah ada dengan melakukan pengujian, lalu membuat pull request agar perubahan tersebut dapat direview dan digabungkan ke dalam branch utama.
3. Untuk berkontribusi pada kode partner saya, saya melakukan proses code review terhadap implementasi yang telah dibuat dan mencari bagian-bagian yang masih dapat diperbaiki. Setelah menemukan beberapa potensi perbaikan, saya membuat branch baru bernama refactor/[NPM] berdasarkan versi terbaru dari repository partner saya. Pada branch tersebut, saya melakukan beberapa perubahan refactoring untuk meningkatkan kualitas kode. Setelah itu, saya memastikan bahwa perubahan yang saya lakukan tidak merusak fitur yang sudah ada dengan melakukan pengujian, lalu membuat pull request agar perubahan tersebut dapat direview dan digabungkan ke dalam branch utama.
4. Selama proses review, saya menemukan beberapa code smell pada kode partner saya. Salah satunya adalah penggunaan field injection menggunakan anotasi @Autowired, yang dapat membuat kode menjadi lebih sulit diuji dan kurang jelas dalam mendefinisikan dependensinya. Selain itu, terdapat penggunaan magic string seperti "address" dan "deliveryFee" yang dituliskan langsung di dalam kode tanpa menggunakan constant. Saya juga menemukan penggunaan dummy data yang di-hardcode di dalam controller, seperti nama produk dan quantity, yang membuat kode menjadi kurang fleksibel jika ingin digunakan dalam kondisi yang lebih nyata.